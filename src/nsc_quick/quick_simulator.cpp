...
QVariant RegisterModel::data(const QModelIndex& idx, int role) const {
    ...
    case HexRole:
        return hex32(values_[i]);
    case DecRole:
        return QString::number(static_cast<int32_t>(values_[i]));
    case ChangedRole:
        return idx.row() == last_written_;
    default:
        return {};
    }
}
...
void QuickSimulator::connectController() {
    connect(ctl_.get(), &nsc::qt::SimulatorController::pipelineStateChanged, this,
            [this](const mips::PipelineState& ps) { onPipeline(ps); });

    connect(ctl_.get(), &nsc::qt::SimulatorController::statisticsUpdated, this,
            [this](const nsc::qt::SimulatorStatistics& s) {
                stats_ = s;
                emit statsChanged();
            });
...
void QuickSimulator::toggleBreakpoint(quint32 pc) {
    if (ctl_->hasBreakpoint(pc))
        ctl_->clearBreakpoint(pc);
    else
        ctl_->setBreakpoint(pc);
...
bool QuickSimulator::isRunning() const {
    return ctl_ && ctl_->isRunning();
}
...
void QuickSimulator::setModel(const QString& m) {
    if (m == model_ || (m != QLatin1String("pipelined") && m != QLatin1String("single"))) return;
    model_ = m;
    rebuildController();
    emit modelChanged();
}
...
void QuickSimulator::onPipeline(const mips::PipelineState& ps) {
    ...
    for (const auto& st : ps.stages) {
        ...
        m.insert(QStringLiteral("text"), st.valid     ? disasm(st.raw, st.pc)
...
void QuickSimulator::appendTrace(const mips::PipelineState& ps) {
    ...
    for (auto& row : trace_) {
        ...
        for (size_t s = 0; s < ps.stages.size(); ++s) {
            ...
            if (st.valid && st.pc == row.pc && cycle >= row.start_cycle &&
                cycle - row.start_cycle < 16) {
        ...
        if (ps.load_stall && tag == QLatin1String("  ") && cycle >= row.start_cycle &&
            cycle - row.start_cycle < 8)
            tag = QStringLiteral("**");
        row.cells << tag;
    }
