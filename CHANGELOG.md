...
### Security
- **ELF32 loader hardened against crafted input.** Every offset and size the parser takes from the
  file is now bounded by the file's real length before use. `p_filesz` previously drove
  `seg.data.resize()` unchecked, so an 88-byte file claiming a 4 GiB segment allocated 4 GiB — this
  is reproducible against the old parser as a libFuzzer out-of-memory and is now rejected in
  milliseconds. Also validated: `e_phentsize` must match the entry size actually read (a mismatch
  used to misparse the table silently), the program-header table must fit in the file (which caps
  `e_phnum` without an arbitrary constant), `p_memsz >= p_filesz`, and `p_vaddr + p_memsz` must not
  wrap the 32-bit address space — a wrapping segment passed `Memory`'s bounds check and overwrote
  low memory. (#127)
- A pure `.bss` segment (`p_filesz == 0`, `p_memsz > 0`) is now mapped and zero-filled instead of
  being skipped entirely, and the zero-fill loop checks `write_byte`'s result like the file-content
  loop does, so a `.bss` running past the end of memory reports the same error instead of
  truncating silently. (#127)
- **libFuzzer harness for the ELF32 loader** (`tests/fuzz/fuzz_elf_loader.cpp`), the only parser in
  `mips_core` that sizes allocations from its own input. Ships with an eight-file seed corpus
  (`tests/fuzz/corpus/elf/`, regenerated by `tests/fuzz/make_elf_corpus.py`) covering valid
  executables, a pure `.bss` segment, `ET_REL`, truncated headers and the 4 GiB reproducer above.
  Built and run by ClusterFuzzLite alongside `fuzz_hex_loader`. (#193)
