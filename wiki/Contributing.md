...
| `cflite_pr.yml`             | PRs that touch `src/`, `include/`, or `tests/fuzz/`             | Runs 120 seconds of libFuzzer fuzzing via ClusterFuzzLite on `fuzz_hex_loader` and `fuzz_elf_loader` (see below) |
...
|-------------------------|----------------------------|---------------------------------------------------------------|
| `fuzz_hex_loader.cpp`   | `mips::parse_hex_program`  | Whitespace-delimited hex words from a file or the Program Loader tab |
...
`fuzz_elf_loader` ships a seed corpus at `tests/fuzz/corpus/elf/`, regenerated by `python3 tests/fuzz/make_elf_corpus.py`. Fuzzing a binary format from an empty corpus burns most of the budget rediscovering the magic bytes and header layout, so the seeds cover valid executables, a pure `.bss` segment, `ET_REL`, truncated headers, and the crafted 4 GiB-allocation input that used to drive the parser out of memory. `build.sh` packs the directory into `$OUT/fuzz_elf_loader_seed_corpus.zip`, which ClusterFuzzLite unpacks automatically.

ClusterFuzzLite (`.clusterfuzzlite/`) provides the OSS-Fuzz-compatible project config, Dockerfile, and `build.sh`. The `cflite_pr.yml` workflow builds and runs both harnesses for 120 seconds under the trigger listed in [CI workflows](#ci-workflows) above. Crashes, hangs and out-of-memory conditions are reported as CI failures.
