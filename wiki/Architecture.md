...
**Fuzz testing**: `tests/fuzz/` holds two libFuzzer harnesses — `fuzz_hex_loader.cpp` targeting `mips::parse_hex_program` and `fuzz_elf_loader.cpp` targeting `mips::parse_elf`, the ELF32 parser that takes its allocation sizes and file offsets from the input itself. Neither is part of the normal CTest runs; both are built and exercised by the ClusterFuzzLite CI workflow (`.github/workflows/cflite_pr.yml`). See [Contributing § CI workflows](Contributing#ci-workflows) for the trigger condition and [Contributing § Fuzzing](Contributing#fuzzing) for the seed corpus and how to add new harnesses.
